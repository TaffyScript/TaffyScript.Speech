...
namespace TaffyScript.Speech
{
    ...
    public class SpeechRecognizer : ITsInstance
    {
        ...
        public TsObject Call(string scriptName, params TsObject[] args)
        {
            switch(scriptName)
            {
                ...
                case "emulate_recognize":
                    return emulate_recognize(null, args);
                case "emulate_recognize_async":
                    return emulate_recognize_async(null, args);
                case "load_grammar":
                    return load_grammar(null, args);
                case "load_grammar_async":
                    return load_grammar_async(null, args);
                case "query_setting":
                    return query_setting(null, args);
                case "recognize":
                    return recognize(null, args);
                case "recognize_async":
                    return recognize_async(null, args);
                case "recognize_async_cancel":
                    return recognize_async_cancel(null, args);
                case "recognize_async_stop":
                    return recognize_async_stop(null, args);
                case "request_recognizer_update":
                    return request_recognizer_update(null, args);
                case "subscribe_event":
                    return subscribe_event(null, args);
                case "update_setting":
                    return update_setting(null, args);
                case "unload_all_grammars":
                    return unload_all_grammars(null, args);
                case "unload_grammar":
                    return unload_grammar(null, args);
        ...
        public bool TryGetDelegate(string delegateName, out TsDelegate del)
        {
            switch (delegateName)
            {
                case "dispose":
                    ...
                    return true;
                case "emulate_recognize":
                    del = new TsDelegate(emulate_recognize, "emulate_recognize", this);
                    return true;
                case "emulate_recognize_async":
                    del = new TsDelegate(emulate_recognize_async, "emulate_recognize_async", this);
                    return true;
                case "load_grammar":
                    del = new TsDelegate(load_grammar, "load_grammar", this);
                    return true;
                case "load_grammar_async":
                    del = new TsDelegate(load_grammar_async, "load_grammar_async", this);
                    return true;
                case "query_setting":
                    del = new TsDelegate(query_setting, "query_setting", this);
                    return true;
                case "recognize":
                    del = new TsDelegate(recognize, "recognize", this);
                    return true;
                case "recognize_async":
                    del = new TsDelegate(recognize_async, "recognize_async", this);
                    return true;
                case "recognize_async_cancel":
                    del = new TsDelegate(recognize_async_cancel, "recognize_async_cancel", this);
                    return true;
                case "recognize_async_stop":
                    del = new TsDelegate(recognize_async_stop, "recognize_async_stop", this);
                    return true;
                case "request_recognizer_update":
                    del = new TsDelegate(request_recognizer_update, "request_recognizer_update", this);
                    return true;
                case "subscribe_event":
                    del = new TsDelegate(subscribe_event, "subscribe_event", this);
                    return true;
                case "update_setting":
                    del = new TsDelegate(update_setting, "update_setting", this);
                    return true;
                case "unload_all_grammars":
                    del = new TsDelegate(unload_all_grammars, "unload_all_grammars", this);
                    return true;
                case "unload_grammar":
                    del = new TsDelegate(unload_grammar, "unload_grammar", this);
                    return true;
                case "unsubscribe_event":
        ...
        public TsObject emulate_recognize(ITsInstance inst, TsObject[] args)
        {
            if(args.Length == 2)
        ...
        public TsObject emulate_recognize_async(ITsInstance inst, TsObject[] args)
        {
            if(args.Length == 2)
        ...
        public TsObject load_grammar(ITsInstance inst, TsObject[] args)
        {
            var grammar = ((Grammar)args[1]).GrammarBuilder;
            grammar.Culture = _source.RecognizerInfo.Culture;
            _source.LoadGrammar(new System.Speech.Recognition.Grammar(grammar) { Name = (string)args[0] });
            return TsObject.Empty();
        }

        public TsObject load_grammar_async(ITsInstance inst, TsObject[] args)
        {
            var grammar = ((Grammar)args[1]).GrammarBuilder;
            grammar.Culture = _source.RecognizerInfo.Culture;
            _source.LoadGrammar(new System.Speech.Recognition.Grammar(grammar) { Name = (string)args[0] });
        ...
        public TsObject unload_all_grammars(ITsInstance inst, TsObject[] args)
        {
            _source.UnloadAllGrammars();
            return TsObject.Empty();
        }

        public TsObject unload_grammar(ITsInstance inst, TsObject[] args)
        {
            var grammar = _source.Grammars.FirstOrDefault(g => g.Name == (string)args[0]);
            if (grammar is null)
                return false;
            _source.UnloadGrammar(grammar);
            return true;
        }

        public TsObject query_setting(ITsInstance inst, TsObject[] args)
        {
        ...
        public TsObject update_setting(ITsInstance inst, TsObject[] args)
        {
            try
            {
                ...
                if (value.Type == VariableType.Real)
                    _source.UpdateRecognizerSetting(name, (int)value.GetFloatUnchecked());
                else if (value.Type == VariableType.String)
                    _source.UpdateRecognizerSetting(name, value.GetStringUnchecked());
                else
                    return false;
                return true;
            }
        ...
        public TsObject recognize(ITsInstance inst, TsObject[] args)
        {
        ...
        public TsObject recognize_async(ITsInstance inst, TsObject[] args)
        {
            if (args.Length == 0)
        ...
        public TsObject recognize_async_cancel(ITsInstance inst, TsObject[] args)
        {
            _source.RecognizeAsyncCancel();
            return TsObject.Empty();
        }

        public TsObject recognize_async_stop(ITsInstance inst, TsObject[] args)
        {
            _source.RecognizeAsyncStop();
            return TsObject.Empty();
        }

        public TsObject request_recognizer_update(ITsInstance inst, TsObject[] args)
        {
